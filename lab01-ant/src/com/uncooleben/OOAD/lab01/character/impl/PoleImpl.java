 ...
 *
 * <p>This is a part of OOAD-Lab01 project.
 *
 * @author Juntao Peng
 */
public class PoleImpl implements Pole {
   ...
   *
   * <p>This constructor should not be called explicitly. Use PoleFactory class's static method
   * 'create' instead.
   *
    ...
    for (Ant ant : this.ants) {
      if (ant.isAlive()) {
        if (ant.getDirection().equals(Direction.LEFT)) {
          ...
          //					System.out.println(
          //							"Ant " + ant.getName() + " moving " + ant.getDirection() + " to " +
          // ant.getLocation());
        } else {
          ant.setLocation(ant.getLocation() + ant.getSpeed() * time / 1000);
          //					System.out.println(
   ...
   * @param needCheck A java.util.Map object from Ant to Boolean. The boolean indicates whether the
   *     key (ant) needs to be checked. Only when the ant is alive is the ant needed to be checked
   */
    ...
    for (Ant otherAnt : needCheck.keySet()) {
      if (!ant.equals(otherAnt) && needCheck.get(otherAnt) == true) {
        if (isCollided(ant, otherAnt,timeGap)) {
          System.out.println(
              ">>" + otherAnt.getName() + " collides at location " + otherAnt.getLocation());
  ...
  private boolean isCollided(Ant ant1, Ant ant2,long timeGap) {
    ...
    // game's timeGap.
    return Math.abs(ant1.getLocation() - ant2.getLocation()) <= this.maxAntSpeed * 2 * 1 / 1000000;
  }

  /**
   * Performs a alive check for a chosen ant. If it is out of the pole bound, it should be killed.
   *
   * @param ant An Ant object of the chosen ant
   */
  ...
  private boolean isOutOfBound(Ant ant) {
    return Math.abs(ant.getLocation()) < this.maxAntSpeed * 1 / 1000000
        || Math.abs(ant.getLocation() - this.size) < this.maxAntSpeed * 1 / 1000000
        || ant.getLocation() < 0
